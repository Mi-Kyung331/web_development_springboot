...
import me.kangmikyung.springbootdeveloper.dto.ArticleResponse;
import me.kangmikyung.springbootdeveloper.dto.UpdateArticleRequest;
import me.kangmikyung.springbootdeveloper.service.BlogService;
...
public class BlogApiController {
    ...
    // HTTP 매서드가 POST 일 때 전달 받은 URL과 동일하면 지금 정의하는 메서드와 매핑
    ...
    /*
        ...
        3. @RequestBody : HTTP 요청을 할 때, 으앋ㅂ에 해당하는 값을 @RequestBody 애너테이션이 붙은
        ...
        API가 잘 동작하는지 하나 테스트를 해볼 예정
            ...
            SQL statement 입력창에(SQL 편집기모양인곳)
            SELECT * FROM article
            그 뒤 Run 눌러서 쿼리 실행
            h2 데이터베이스에 저장된 데이터를 확인 할 수 있습니다.
            애플리케이션을 실행하면 자동으로 생성한 엔티티 내용을 바탕으로 테이블이 생성되고,
            우리가 요청한 POST 요청에 의해 INSERT문이 실행되어 실제로 데이터가 저장된 것 입니다.
    ...
    /*
        @PathVariable : URL에서 값을 가져오는 애너테이션.
            ...
            그리고 그 글을 찾으면 3번 글의 정보(제목/내용)를 body 담아서 웹브라우저에 가지고 옵니다.
     */
