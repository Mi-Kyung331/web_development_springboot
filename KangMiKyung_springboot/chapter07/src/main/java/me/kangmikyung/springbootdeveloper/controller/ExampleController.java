...
/*
    ...
    모델 객체는 따로 생성할 필요 없이 코드처럼 argument로 선언하면 스프링이 알아서 만들어주므로 편리하게 사용 가능.

    "person"이라는 키에 사람 정보를, "today"라는 키에 날짜 정보를 저장.

    thymeleafExample()이라는 메서드는 "example"이라는 리턴값을 가지는데,
    얘가 @Controller라는 애너테이션과 합쳐지면 -> 뷰의 이름이라는 의미가 됩니다.
    ...
    모델 역할 살펴 보면
        ...
        컨트롤러는 모델을 통해서 (앞으로 자주 쓸 거다.) 데이터를 설정하고,
        모델은 뷰로 해당 데이터를 전달해서 키에 맞는 데이터를 뷰에서 조회할 수 있게 만들어줍니다.

        즉, 모델은 컨드롤러와 뷰의 중간다리 역할을 해준다고 생각하시면 됩니다.
                                                    "person" id: 1
