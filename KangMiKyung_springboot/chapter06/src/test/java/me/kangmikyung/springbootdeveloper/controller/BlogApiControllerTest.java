package me.kangmikyung.springbootdeveloper.controller;
...
class BlogApiControllerTest {
        ...
        : 이 클래스로 만든 자바 객체를 JSON 데이터로 변환하는 것 -> 직렬화
            ...
            형식에 맞게 변환하는 과정이 필요합니다. 이 작업을 직렬화/역직렬화
                ...
                1) 직렬화 Java 시스템 내부에서 사용되는 객체를 외부에서 사용하도록
                    변환하는 과정을 뜻하는데,
                    예를 들어 title은 "제목, content 는 "내용"이라는 값이 들어가 있는 개체가 있다고 가정했을 때,
                    자바 상에서는
            ...
            When - 블로그 글 추가 API에 요청을 보냅니다.
                    요청 타입은 JSON이며, given 절에서 미리 만들어둔 객체를 요청 본문으로 함께 보냅니다.
    ...
    public void addArticle() throws Exception {
        ...
        List<Article> articles = blogRepository.findAll();  // repository에서 전체 글(객체)를 가져와 list에 담았습니다.
        ...
        /*
            ...
            writeValueAsString() a메서드 : 객체를 JSON으로 직렬화.
            그 이후에 MockMvc를 사용해 HTTP 메서드, URL, 요청본문, 요청 타입 등을 설정 뒤 테스트 요청 보냈습니다.
            contentType() 메서드 : JSON / XML 등 다양한 타입 중 하나를 선택할 수 있는데, 저희는 JSON을 사용했습니다.
            assertThat() 메서드 : 글의 개수가 1인지,
                Article 객체의 field인 title의 값이 "제목"인지,
                Article 객체의 field인 content의 값이 "내용"인지 확인했습니다.
    ...
    public void deleteArticle() throws Exception {
        ...
        mockMvc.perform(delete(url, savedArticle.getId())) // findArticle()처럼 하나의 객체를 대상을 할 때는 argument가 url만 있는게 아닙니다.
                .andExpect(status().isOk());

        // then - 검증단계에9서 블로그 글 목록을 전부 다 가지고 올겁니다. -> size()체크 했더니 0 이거나 / .isEmty() 메서드 활용
        // 전부 다 가지고 오는 단계
