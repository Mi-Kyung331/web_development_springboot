...
class MemberRepositoryTest {
    ...
    /*
        이제는 id가 아닌 name으로 찾기로 해보겠습니다.
        id = 기본키
        name의 경우에는 칼럼이 있을 수도 없을 수도 있기 때문에 JPA에서 기본으로 name을 찾아주는 매서드가 없습니다.

        필수 컬럼이 나는 것을 조건으로 검색을 하기 위해서는 MemberRepository.java에서
        메서드를 추가할 필요가 있습니다.

        MemberRepository.java로 이동

        MemberRopository에 findByName 정의 후에 이하에 작성
     */
    ...
    void saveMember() {
        ...
        // when
        memberRepository.save(member);  //save가 자동완성되었다는 점에도 주목해야 합니다.
