...
1. 사전 지식 : API와 REST API
    ...
    식당으로 알아보는 API
        여러분이 손님인데, 식당을 들어가게됨 -> 점원에게 요리를 주문합니다. (주방으로 달려가서 요리를 주문하는게 아닌)
        ...
        손님 <-----> 점원 <-----> 주방
          ...
          우리는 웹 사이트의 주소를 입력해서 '구글 메인 화면을 보여주'라고 요청하면, API는 이 요청을 받아서 서버에게 전달.
    ...
    웹의 장점을 최대한 활용하는 형태라고 알려진 REST API
        ...
        URL의 설계 방식
            ...
            단점 :
                HTTP 매서드, 즉 GET, POST와 같은 방식의 개수에 제한이 있음.
                설계를 위해 공식적으로 제공되는 표준 규약이 없음.

            장단점을 고려했을 때, 주소와 HTTP 매서드만 보고 요청의 내용을 파악할 수 있다는 장점으로
            ...
            REST API를 사용하는 방법
                규칙 1. URL에는 동사를 쓰지 말고, 자원을 표시한다.
                    *자원 : 가져오는 데이터를 의미. 예를 들어 학생 중에 id가 1인 학생의 정보를 가져오는 URL은
                        1번 예시 : /students/1
                        2번 예시 : /get-student?student_id=1
                    과 같은 방식으로 설계할 수 있는데

                    이 중 더 REST API에 맞는 방식은 1번예시에 해당함.
                    2번예시의 경우 자원이 아닌 다른 표현을 섞어 사용했기 때문 (get)
                ...
                규칙 2. 동사는 HTTP 메서드로
                    ...
                    예를 들어 블로그에 글을 쓰는 설계를 한다고 가정했을 때
                        ...
                        2) 블로그 글을 '추가'하는 API         : POST/articles > 추가ㅏㄱ 완료되기 전이라 DB상 id가 없음
                    ...
                    * GET / POST 등은 URL에 입력하는 값이 아니라 내부적으로 처리하는 방식을 미리 정하는 것으로
                    ...
                    이외에서 '/'는 계층 관계를 나타내는 데 사용하거나, 밑줄 대신 하이픈을 사용하거나, 자원의 종류가
