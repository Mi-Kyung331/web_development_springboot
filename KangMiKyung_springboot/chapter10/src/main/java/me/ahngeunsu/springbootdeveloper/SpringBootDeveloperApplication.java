...
사전 지식 : OAuth
    ...
    관련 용어 정리
        ...
        리소스 서버(Resource Server) : 리소스 오너의 정보를 가지며, 리소스 오너의 정보를 보호하는 주체를 의미하고,
            네이버, 구글, 카카오, 페이스북이 리소스 서버에 해당.
        인증 서버(Authorization Server) : 클라이언트에게 리소스 오너의 정보에 접근 할 수 있는
    ...
    OAuth를 사용하면 인증 서버에서 발급받은 토큰을 사용해서 리소스서버에
     ...
     1. 권한 요청    |       5. 액세스 토큰으로 데이터에 접근      | 인증 서버에
            ...
            1. 권한 요청 :
                ...
                즉 카카오나 구글 권한 서버에 요청을 보내는 거라고 볼 수 있습니다. 요청 URL는 권한 서버마다 다르지만
            ...
            3. 인증 코드 제공
                사용자가 로그인에 성공하는 권한 요청 시에 파라미터로 보낸 redirect_uri로 리다이렉션됩니다.
                이 때 파라미터에 인증 코드를 함께 제공합니다.
    ...
    쿠키 관리 클래스를 구현할겁니다.
        OAuth2 인증 플로우를 구현한면서 쿠키를 사용할 일이 생깁니다.
        그 때 마다 쿠키를 생성하고 삭제하는 로직을 추가하면 불편해서 유틸리티로 사용할 쿠키 관리 클래스를 미리 구현

        springbootdeveloper 패키지 내에 util 패키지를 생서 -> CookieUtil.java를 생성
 */