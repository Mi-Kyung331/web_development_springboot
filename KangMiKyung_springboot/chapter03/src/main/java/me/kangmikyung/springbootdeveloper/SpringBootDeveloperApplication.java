...
/*
    1. 스프링 부트 3 구조 살펴보기
        ...
        계층 : 각자의 역할과 책임이 있는 어떤 소프르웨어의 구성 요소
        ...
        스프링 부트에서의 계층
            ...
            2) 비지니스(Business)
                모든 비지니스 로직을 처리
                    * 비지니스 로직 : 서비스를 만들기 위한 로직을 의미하며, 웹 사이트에서 벌어지는 모든 작업
                        ...
                        주문 처리를 하다가 발생하는 예외 처리 로직, 주문을 받거나 취소하는 등 프로세스를 구현하기 위한 로직 등을 의미
                    서비스가 비지니스 계층의 역할을 함
            ...
            🎈 계층은 개념의 영억이고 Controller, Service, Repository는 실제 구현을 위한 영억
                -> 추후 수업(아마 목요일) 예정입니다.
        ...
        스프링 부트 프로젝드 디렉토리 구성
            : 스프링 부트에는 정해진 구조가 없지만, 추천 프로젝트 구조가 있고, 이를 많은 개발자들이 따르도록
                ...
                main : 실제 코드를 작성하는 공간. 프로젝트 실행에 필요한 소스 코드나 리소스 파일은 모두 이 폴더 안에 들어있음
                    (프로젝트 생성시 자동 구현 됨)

                test : 프로젝트의 소스 코드를 테스트할 목적의 코드나 리소스 파일이 들어있음
                    (원래는 자동생성되는데 어제 제가 만들었을 때 안만들어지는 걸로 보아 설정따라 다른 듯 합니다.)
    ...
    지시사항
        main 디렉토리 구성하기
            : main 디렉토리 내에 java와 resources로 구성되어 있음. 여기에 아직 추가하지 못했던
                스프링 부트 프로젝트의 구성 요소를 추가합니다.

            01 단계 - HTML과 같은 뷰 관련 파일(chapter 07에서 수업 예정)을 넣을 templates 디렉토리
            ...
            03 단계 - 스프링 부트 설정을 할 수 있는 application.yml 파일을 생성
                ...
                데이터베이스의 설정 정보, 로깅 설정 정보 등이 들어갈 수도 있고, 직접 정의할 때 사용하기도 함.
                앞으로 프로젝트를 진행하며 자주 사용할 파일입니다.
                resources 우클릭 -> new file -> application.yml

    2. 스프링 부트 3 프로젝트 발전시키기

        각 계층의 코드를 추가할 예정 -> 이를 통해 계층이 무엇이고, 스프링 부트에서는 계층을 어떻게 나누는지를
            감을 잡아나가실 수 있도록 수업하겠습니다. (목요일에 이루어질 예정)
        지금은 의존성을 추가한 다음에 -> 프레젠테이션 계층, 비즈니스 계층, 퍼시스턴스 계층 순서로
            코드를 추가할 예정입니다.
        ...
        implementation : 프로젝트 코드가 컴파일 시점고 ㅏ런타임에 모두 해당 라이브러리를 필요로 할 때
    ...
    지시사항
        프레젠테이션, 서비스, 퍼시스턴스 계층 만들기
            ...
            3. 퍼시스턴스 계층 코드 작성 -> 같은 위치에 Menber.java 생성하세요. -> 실제 DB에 접근하는 코드 작성
