        ...
        * 토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
        ...
        토큰 기반 인증의 특징
            1) 무상태성 :
                ...
                서버가 테이터를 유지하고 있으려면 그만큼 자원을 보시해야하는데,
            ...
            2) 확장성 :
                ...
                토큰을 가지는 주체가 서버가 아닌 클라이언트이기 때문에 가지고 있는 하나의 토큰으로
                결제 서버와 주문 서버에게 요청을 보내는 것이 가능.
                추가로 페이스북 로그인, 구글 로그인과 같이 토큰 기반 인증을 사용하는
                다른 시스템에 접근해 로그인 방식을 확장할 수도 있고,
                이를 활용해 다른 서비스에 권한을 공유할 수도 있음.

            3) 무결성 :
                토큰 기반 인증 방식은 HMAC(Hash-based Message Authentication) 기번이라고도 하는데,
                토큰을 발급한 이후에는 토큰 정보를 변경하는 행위를 할 수 있음.
                만약 토큰을 한 글자라도 변경하면 서버에서는 유효하지 않은 토큰이라고 판단합니다.


        JWT(JSON Web Token) -> JSON형태로 주고받는 거죠.
            발급받은 JWT를 이용해 인증을 하려면 HTTP 요청 헤더 중에 Authorization 키 값에
            Bearer + JWT 토큰 값을 넣어 보내야 합니다.

            JWT 구조 : 온점(.)을 기준으로 헤더(header), 내용(payload), 서명(signture)로 이루어짐.
                aaaa . bbbb . cccc
                헤더    내용    서명
                ...
                2) 내용에는 토큰과 관련된 정보를 담습니다. 내용의 한 덩어리를 클레임(claim)이라고 하며,
                    키-값의 한쌍으로 이루어집니다.
