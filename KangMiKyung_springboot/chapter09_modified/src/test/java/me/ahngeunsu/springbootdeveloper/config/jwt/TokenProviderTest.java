...
/*
    ...
    2.
        ...
        given : jjwt 라이브러리를 사용, 토큰 생성.
            -> 만료 시간은 현재 시간으로부터 14일 뒤로 만료되지 않은 토큰으로 생성
            -> Default로 만들었습니다. -> JwtFactory.java

        when : 토큰 제공자의(tokenProvider) validToken() 메서드를 호출해 유효한 토큰인지 검증 한 뒤
            결과값을 boolean 값으로 반환
        ...
        given : jjwt 라이브러리를 사용, 토큰 생성.
        -> 만료 시간은 1970년 1월 1일부터 현재 시간을 밀리초 단위로 지환한 값(new Date().getTime())에
            1000을 빼서 이미 만료된 토큰으로 설정.
    ...
    3. getAuthentication()
        given : jjwt 라이브러리를 사용해 토큰 생성, 이 때 subject는 "user@gmail.com"
    ...
    4. getUserId()
        given : jjwt 라이브러리를 사용해 토큰 생성. 이 때 클레임을 추가하여 키는 "id" 값은 1인 유저 ID를 생성

        when : getUserId()를 호출하여 ID를 반환
