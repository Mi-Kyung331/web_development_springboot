...
-- 예제 : 회원 정보 테이블 users에서 거주 국가(country)가 한국(korea)인 회원만 추출하는 쿼리문
...
-- 거주 국가가 한국이면서 회원아이디가 10인 회원만 추출
SELECT *
	FROM users
	WHERE country = "Korea" AND id = 10;	-- id가 PK이기 때문에 인위적인 표현
	
-- WHERE 절에서는 여러 조건을 동시에 적용할 수 있고, 조건의 개수에 제한은 없음.
-- 논리연산 AND / OR / BETWEEN

	
-- 예제 : 회원 정보 테이블 users에서 가입 일시(creat_at)가
...
-- LIKE : 해당 전치사 뒤의 작은따옴표 / 큰따옴표 내에서는 와일드 카드를 사용 가능
-- SQL을 해석하는 컴퓨터가 LIKE 코드를 읽는 순간 와일드카드를 감지하는데 
-- SQL에서의 와일드카드는 '%'로 0개 이상의 임의의 문자열을 의미하는 메타문자(metacharacter)로 사용됨
...
-- 연습 문제
	-- 1. users에서 country가 Mexico인 회원 정보 추출
		-- 단 created_at, phone, city, country 컬럼만 추출 할 것
	SELECT created_at, phone, city, country
		FROM users 
		WHERE country = "Mexico";
	
	-- 2. products에서 id가 20이하고 price는 30이상인 제품 정보 출력
	...
	-- 3. users에서 country가 Korea, Canada, Belgium도 아닌 회원의 정보를 모두 추출할 것
...
-- 이상의 경우 저장된 순서대로 정렬된 결과만 볼 수 있습니다. -> id라는 PK갑에 따라
...
-- 정렬할 때 사용 : 숫자의 경우는 쉽게 알 수 있지만, 
-- 문자의 경우는 아스키 코드(ASCII Code)를 기준으로 합니다.
...
-- 예제 : users에서 username, phone, city, country, id 컬럼을 순서대로 출력 하는데,
...
-- 즉, ORDER BY는 데이터 추출이 끝난 후를 기점으로 적용이 된다는 점을 명심해야하 합니다.
...
-- 2개 이상의 정렬 기준을 지정할 때 각각 지정 가능
	-- 이상의 경우 각 컬럼 당 명시적으로 ASC 혹은 DESC를 지정 해주어야 함.
...
SELECT *
	FROM 음식점정보
	WHERE 지역 = "마포구"
		AND 1인분가능여부 = 1;		-- 1을 쓸 경우에는 True의미 / 0은 False의미
