...
select 28 + 891;
-- 2. SELECT를 이용하여 19 x 27의 결과를 표시하라. 단, 칼럼명은 Result로 표시하라.
select 19 * 27 as Result;
-- 3. SELECT를 이용하여 다음 세 가지 결과를 각각 다른 컬럼으로 결과 창에 표시하라.
-- 37 + 172 (단, 컬럼명은 Plus)
-- 25 x 78 (컬럼명은 Times)
-- I love SQL (칼럼명 Result) 
select 
	37 + 172 as Plus,
	25 * 78 as Times,
...
-- 연습문제 2
-- 제품 정보 테이블 products에 있는 모든 테이터를 출력하시오
...
-- products에서 가격(price), 할인가(discount_price) 칼럼을 10개만 출력합니다.
select price, discount_price 
	from products
	limit 10;
-- SQL문의 경우에는 순서가 매우 명확하게 정해져 있는 편이기 때문에
-- 읽기는 쉽지만 익숙해지기 전 까지 직접 작성하는 것이 까다로운 편

-- orderdetails의 모든 정보를 표시하세요
select * from orderdetails;
-- 회원 정보 테이블 user에서 상위 7건만 표시하세요 (전부 다) 
