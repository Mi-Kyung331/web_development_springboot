...
-- 전체 데이터를 째로 함수 적용하는 경우도 있지만, 그룹별로 수차를 도출하는 일도 있는데,
...
-- 집계 함수로 계산한 결과 중 조건에 맞는 데이터만 필요한 경우에는 
...
집계함수는 여러 행의 데이터를 '하나의 결과값'으로 집계하는 반면, 
일반함수는 한 행의 데이터에 하나의 결과값을 반환한다.
...
GROUP BY
	...
	-- 집계함수만으로 원하는 결과를 얻을 수 없을 때(데이터 전체가 아니라,
	-- 원하는 기준으로 그룹을 나눈 상태에서 계산을 해야할 때를 의미)가 있습니다.
	-- 예를 들어, 전체 회원 수가 아닌 국가별 회원 수를 나누어 계산하기도 하며,
	-- 월 별로 가입한 회원 수를 집계하기도 하기 때문.
	...
	-- SQL문을 작성해야 합니다.
	-- 국가 수는 적지도 않고, 실제 환경에서는 훨씬 더 많은 데이터를 다루는데, 
	-- 조건을 일일이 적용하기에는 비효율적입니다.
