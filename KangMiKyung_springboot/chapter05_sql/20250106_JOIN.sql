...
SELECT SUBSTR(created_at, 1, 7) AS month, COUNT(DISTINCT id) AS uniqueUserCnt
	FROM users
	GROUP BY SUBSTR(created_at, 1, 7) 	-- 이건 확인 후에 말씀드리겠습니다. (month로 썼을 경우)
...
-- 이상의 SQL문에 대한 해석
-- 기존에 FROM 다음에는 테이블 명 하나만 작성되었지만, 이제는 JOIN 연산을 위한 추가 문법이 적용되었음.
-- 회원 정보와 주문 정보를 하나로 결합하기 위해 user와 order를 INNER JOIN(추후설명)으로 묶고,
-- '후속조건'으로 "주문 정보가 있는 회원의 정보만 출력하기 위해" u.id = o.user_id를 적용함.

-- 여기서 여러분들에게 연습문제시에만 설명했던 부분을 명확하게 하고 넘어가야 합니다.

-- users PK인 id는 회원id에 해당합니다.
-- orders에 PK인 id는 주문id에 해당하고, 2번째 컬럼인 users_id는
...
-- 이상의 문제에서의 조건은 '주문 정보가 있는 회원의 정보만 출력'하는 것이므로, ORDERS 내게 
-- user_id가 일부 기준이 되어야 합니다.

-- 왜냐하면 users 내에 있는 id는 1부터 끝까지 있으니까요.
-- users 테이블에는 회원 id가 id 컬럼에 기록되어있고, orders 테이블에는 회원 id가 user_id로
...
-- FROM에서 JOIN이 정렬된 후에 단일 테이블에 명령을 내리는 것 처럼 쿼리를 작성 가능
...
-- 세 번 주문했다면 회원 정보(users)와 주문정보(orders)를 결합한 결과에는 u.id = 7인 행에 3개 있을 것이다.
-- 이 경우 '한 번 이라도 주문한 회원 수'를 중복 없이 구하려면 회원 id를 중복 제거한 뒤에 회원 수를 count할 필요가 있다.
...
-- 이름 JOIN 시켰을 때 u.id로 ORDER BY하더라도 합친 테이블 상에서 제 1컬럼(즉 PK컬럼)에 
...
-- LEFT JOIN은 INNER JOIN의 결과값이 교집합 뿐만 아니라 JOIN 명령문의 왼쪽에 위치한 테이블의 모든 결과값을 가져옵니다.
...
-- 한 쪽 데이터의 값을 보존해야 할 때가 많은데, 이번 예시에서도 그렇습니다.
-- 주문 정보가 없는 회원의 정보까지 출력하려면 LEFT JOIN을 활용합니다. (제가 든 예시는 블로그 글을 남긴 user와
-- 한 번도 남기지 않은 user를 구분할 때 였습니다.)
...
-- users와 orders를 하나로 결합해 출력해봅시다.
	-- 단, 주문 정보가 없는 회원의 정보만 출력합니다.
SELECT *
	FROM users u LEFT JOIN orders o ON u.id = o.user_id
	WHERE o.id IS NULL;	-- o.id / o.user_id / o.staff_id, o.order_id

-- users와 order를 하나로 결합하고, 추가로 orderdetails에 있는 데이터도 출력해보자.
	-- 단, 주문 정보가 없는 회원의 주문 정보도 모두 출력하고, 다음 컬럼을 출력하자.
	-- u.id, u.username, u.phone, o.user_id, o.id, od.order_id, od.product_id
	-- JOIN이 두 번 일어납니다.
...
-- users와 orders를 하나로 결합하여 출력해보자.
	-- 단, 회원 정보가 없는 주문 정보도 출력.
SELECT *
	FROM users u RIGHT JOIN orders o ON u.id = o.user_id;

-- RIGHT JOIN은 기본적으로 LEFT JOIN과 기능은 동일합니다. LEFT JOIN에서는 왼쪽에 위치한 
-- 테이블의 모든 값을 가져왔습니다.(즉, JOIN했을 때를 기준으로 u.id는 있지만 o.user_id가 없는 경우도 출력된다는 것을 의미)
...
-- 결합으로 봤을 때 orders는 users 테이블에 종속되어있으므로 null값이 출력될 수 없습니다.
...
-- CROSS JOIN
-- 	: 두 테이블 간의 집합을 조합해 만들 수 있는 모든 경우의 수를 생성하는 방식으로,
...
-- 실제 환경에서는 CROSS JOIN을 제한하는 편입니다.
-- 컴퓨터에 많은 연산을 요구하는데, 실직적으로는 NULL값만 출력되는 경우의 수가 많아 쓸모는 없기 때문입니다.
...
-- 1. users와 staff를 참고하여 회원 중 직원인 사람의 회원 id, 이메일, 거주 도시, 거주 국가, 성, 이름을 한 화면에 출력하세요.
...
-- 4. orders와 orderdetails, products를 참고하여 회원 아이디별 주문 금액의 총 합을
-- 정상 가격과 할인 가격 기준으로 각각 구하세요.
	-- 단, 정상 가격 주문 금액의 총 합 기준으로 내림차순으로 정렬하세요.
SELECT o.user_id,
...
-- JOIN 총 정리
-- JOIN : 두 테이블을 하나로 결할 때 사용(정규화를 통해서 DB 관리를 효율화했기 때문에 하나로 합치게 될 때 사용되는 명령어). 

-- 정리 1. 기본 형식
-- FROM [테이블명1] a(별칭) [INNER / LEFT / RIGHT / CROSS JOIN] [테이블명2] b(별칭) ON [JOIN 조건]
	-- 선호되는 조건 방식 PK = FK

-- JOIN 명령어는 FROM에서 수행됩니다. 쿼리 진행상 FROM이 가장 먼저 수행되므로
-- 일단 JOIN이 수행된 뒤에 다른 명령어들이 수행됩니다.
-- JOIN 사용 시, 결합할 두 테이블 사이에 원하는 JOIN 명령어를 작성하고, 테이블 별칭을 설정(알리아스가 아닙니다.).
...
-- 정리 2. JOIN 중첩
-- FROM [테이블명1] a(별칭) [INNER / LEFT / RIGHT / CROSS JOIN] [테이블명2] b(별칭) ON [JOIN 조건1]
-- 			[INNER / LEFT / RIGHT / CROSS JOIN] [테이블명3] c(별칭) ON [JOIN 조건2]

-- FROM 내에서 JOIN을 여러 번 중첩 사용 가능. 앞의 JOIN의 ON 절 뒤에 새로운 JOIN 명령어를 작성하면
-- 제 3의 테이블과 결합 가능 -> 순서대로 적용한다는 점이 중요합니다.
-- 횟수 제한은 없으나, 테이블 크기에 따라 많은 연산이 필요할 수 있어 사전에 필요한 연산인지 점검할 필요가 있습니다.


-- 정리 3. INNER JOIN
-- FROM [테이블명1] a(별칭) INNER JOIN [테이블명2] b(별칭) ON a.key = b.key
...
-- FROM [테이블명1] a(별칭) LEFT/RIGHT/FULL JOIN [테이블명2] b(별칭) ON a.key = b.key

-- OURTER JOIN은 LEFT OUTER JOIN, RIGHT OUTER JOIN, FULL OUTER JOIN을 포괄하는 용어.
-- OUTER 생략 가능
...
-- 정리 5. CROSS JOIN
-- FROM [테이블명1] a(별칭) CROSS JOIN [테이블명2] b(별칭)
...
-- FULL OUTER JOIN이 경우 ON 조건에 부합할 때만 결과값을 도출하는 반면에,
