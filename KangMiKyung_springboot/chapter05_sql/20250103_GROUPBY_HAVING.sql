...
-- 표시 컬럼 uniqueUserCnt 두개 컬럼으로 구성되어 있습니다. (Group By - x)
...
그 후에 is_marketing_agree를 기준으로 그룹화되었다.
...
-- 4) GROUP BY에서 두 개 이상의 기준 컬럼을 조건으로 추가하여 여러 그룹으로 분할 가능한데
-- 		컬럼 순서에 따라 결과에 영항을 미치므로 우선순위를 생각할 필요가 있음.
...
-- 1. orderdetails에서 order_id 별 주문 수량 quantity의 총합을 출력할 것.
	-- 주문 수량의 총합이 내림차순으로 정력되도록 할 것 (함수는 어제 수업에서 한 것 확인)
SELECT COUNT(order_id), SUM(quantity)
	FROM orderdetails
	GROUP BY order_id
	ORDER BY SUM(quantity) DESC;

-- 2. orders에서 staff_id 별, user_id 별로 주문 건수(COUNT(*))를 출력 할 것
	-- 단, staff_id 기준 오름차순하고 주문건수 별 기준 내림 차순
...
-- SELECT country, COUNT(DISTINCT id) 
-- 	FROM users 
-- 	WHERE COUNT(DISTINCT id) >= 8;		// 오류나는 사례
...
-- 6. ORDER BY를 통해 특정 컬럼 및 연산 결과를 통한 오름 차순 및 내림 차순으로 나열한다.

-- 연습문제
-- 1. orders에서 회원별 주문 건수 추출하라 (단 주문 건수가 7건 이상인 회원의 정보만 추출,
-- 주문 건수 기중으로 내림차순 정렬, user_id와 주문 아이디(id)컬럼 활용할 것)
...
--	국가 별로 추출 (단, 회원 수가 5명 이상인 국가만 추출하고, 회원 수 기준으로 내림차순)
