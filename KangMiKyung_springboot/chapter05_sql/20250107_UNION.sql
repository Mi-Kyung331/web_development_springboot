...
-- 해당 이유는 : UNION은 결합하는 두 결과 집합에 대한 '중복 제거 기능'이 포함되어있습니다.
...
-- 참고 UNION ALL 사용시 SELECT에서 컬럼 션별 예시
(SELECT id, phone, city, country		-- 일부 컬럼만 지정해서 출력
	FROM users)
	UNION ALL 
(SELECT id, phone, city, country		-- 기준이 되는 첫 번째 SELECT절에서 선택하는
	FROM users)							-- 컬럼의 종류 및 개수가 완벽히 일치해야 합니다.
	ORDER BY id;

-- 실무에서는 UNION ALL이 더 권장됩니다. UNION의 경우 중복 제거가 있는데,
-- 대량의 데이터를 대상으로 중복 제거할 때 컴퓨터에 무리한 연산 부하를 줄 가능성이 있기 때문에
-- 일단 UNION ALL을 통해서 최종 결과 형태 확인 후에 -> UNION을 적용하는 식으로 프로세스가 짜여있는 편

-- users에서 country가 Korea인 회원 정보만 추출하고(1번추출),
-- Mexico인 회원 정보만 추출해서(2번추출) 결합해보자.
	-- 단, 컬럼은 id, phone, city, country만 출력하고, 최종 결과 집합은 country 기준 알파벳순
...
-- 이렇게 작성하지 마세요
SELECT id, phone, city, country FROM users WHERE country IS IN ("Korea", "Mexico");
...
-- 두 결과 집합을 UNION ALL을 사용해 하나로 결합하세요
	-- 단, 최종 결과는 최신순으로 정렬
-- 1번 쿼리
...
-- YYYY-MM-DD 형식으로 지정되어 있다면, 문자열 비교 결과와 실제 날짜 비교 결과가 동일하게 작용함.
-- 그래서 MM-DD-YYYY 형태로 되어있다면 오류가 발생할 가능성이 있습니다.


-- 2. users에서 USA에 거주 중이면서 마케팅 수신에 동의(1)한 회원 정보와 France에 거주 중이면서
-- 마케팅 수신에 동의하지 않은(0) 회원 정보를 SELECT로 각각 추출하고, 두 결과 집합을 UNION ALL을 사용해 하나로 결합하라.
	-- 단, 최종 결과는 id, phone, country, city, is_marketing_agree 컬럼을 추출하고,
	-- 거주 국가 기준으로 알파벳 역순으로 추출하세요.
...
-- UNION을 활용하여 orderdetails와 products를 FULL OUTER JOIN 조건으로 결합하여 출력하세요.
...
-- 서브쿼리는 작성한 쿼리를 소괄호로 감싸서 사용하는데, 실제로 테이블은 아니지만 테이블처럼 사용이 가능합니다.

-- products에서 name(제품명)과 price(정상가격)을 모두 불러오고,
...
-- orders와 sfaff를 활용해 last_name이 Kyle이나 Scott인 직원의 담당 주문을 출력하려면?
	-- 단, 서브쿼리 형태를 활용하자
SELECT id
	FROM staff
	WHERE last_name IN ("Kyle", "Scott");		-- 조건절에 쓰일 경우에 scalar서브가 아니었다는 점에 주목
...
-- WHERE에서 IN 연산자와 함께 서브쿼리를 활용할 경우 :
...
-- 이상의 쿼리문의 해석 :
	-- 직원 자신이 자기 쇼핑몰에서 주문한 이력이 반환된 것.


-- products에서 discount_price가 가장 비싼 제품 정보 출력하세요.
	-- 단, producsts의 전체 컬럼이 다 출력되어야 합니다.
...
-- 주문 상세 정보 테이블 orderdetails에서 quantity가 50이상인 정보를 각각 서브쿼리로 작성하고,
-- INNER JOIN하여 출력해봅시다.
...
-- 3) 1과 2의 INNER JOIN 구문 -> FROM절에서 이루어 집니다.
SELECT *
	FROM (
		SELECT * 
			FROM orders 
			WHERE order_date >= "2015-07-01"
				AND order_date <= "2015-08-01") o 		-- 1의 결과가 테이블이었기 때문에 별칭 o를 사용
INNER JOIN 
		(SELECT *
			FROM orderdetails 
...
-- 서브쿼리를 작성하기 위한 방안 중에 하나는 서브 쿼리에 들어가게 될 쿼리문을 작성한 결과값을 확인.
-- 이후 해당 쿼리가 scalar냐 아니냐에 따라서 그 위치 역시 어느 정도 통제 가능
-- ex) scalar인 경우에는 select절에 들어가는 것 처럼
...
-- 여러 개의 컬럼을 추가하고 싶을 때는 서브쿼리를 여러 개 작성하면 됩니다.
-- 특징 :
	-- SELECT의 서브 쿼리는 메인 쿼리의 FROM에서 사용된 테이블이 아닌 테이블도 사용이 가능하기 때문에
	-- 불필요한 조인 수행을 줄일 수 있다는 장점이 있다.
...
-- FROM에서 사용되는 서브쿼리 :
	-- 인라인 뷰, 마치 테이블처럼 서브 쿼리의 결과값을 사용 가능.
	-- 또한 FROM에서 2개 이상의 서브쿼리를 활용하여 JOIN연산 가능
	-- 이 때 조인 연산을 위해 별칭 생성 가능한데 서브 쿼리가 끝나는 괄호 뒤에 공백을 한 칸 주고,
	-- 원하는 별칭을 쓰면 된다. (orders o / orderdetails od와 같은 방식)
	
-- 특징 : 
	-- FROM에서 서브쿼리를 적절히 활용하면 적은 연산으로 같은 결과를 도출 가능. 단, RDBS기준
	-- 테이블 검색을 빠르게 할 수 있는 인덱스* 개념이 있는데 서브 쿼리를 활용하면 인덱스를 사용하지 못하는 경우가 있으므로 주의해야 함.

-- 인덱스(index) :
	-- 테이블의 검색 속도를 높이는 기능으로, 컬럼 값을 정렬하여 검색시 더욱 빠르게 찾아내도록 하는 자료 구조
...
-- WHERE에서 필터링을 위한 조건 값을 설정하는데 서브쿼리 사용 가능
-- 위의 예시에서는 IN 연산자를 사용했지만, 다른 비교 연산자도 사용 가능

-- 특징 :
	-- IN 연산자의 경우에 다중 컬럼 비교를 할 때는 서브쿼리에서 추출하는 컬럼의 개수와
	-- WHERE에 작성하는 필터링 대상 컬럼 개수가 일치 해야 함. 
		-- 이 때 필터링 대상 컬럼이 2개 이상이면 ()로 묶어서 작성해야 합니다.
